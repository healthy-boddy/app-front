...
const AnalyseSingleScreen = ({routes}) => {
    ...
    const [activeAnalyzeId, setActiveAnalyzeId] = useState(
        analyzeFromStorage?.id
    );
    ...
    const renderItem = ({item}) => {
        return (
            <View style={styles.item_box}>
                ...
                <Text style={styles.item_indicator}>
                    {item.value} {item.unit_info?.unit_name}
                </Text>
            </View>
        );
    };
        ...
        <MainContainer>
            <View style={{flex: 1}}>
                ...
                <View style={{marginTop: 16}}>
                    <Text>{analyse?.date}</Text>
                    <Description marginTop={8}>{analyse?.laboratory_name}</Description>
                </View>
                <View style={styles.items}>
                    <FlatList data={indicators} renderItem={renderItem}/>
                    {analyse?.photo && (
                        <View>
